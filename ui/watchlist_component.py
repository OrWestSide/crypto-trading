...
from ui.styling import (BG_COLOR, FG_COLOR, BOLD_FONT, BG_COLOR_2, FG_COLOR_2, GLOBAL_FONT, BUTTON_DELETE_COLOR)


class Watchlist(tk.Frame):
    def __init__(self, binance_contracts: Dict[str, Contract], bitmex_contracts: Dict[str, Contract], *args, **kwargs):
    ...
    def _add_binance_symbol(self, event):
        ...
        if symbol in self.binance_symbols:
            self._add_symbol(symbol, Exchange.binance)
    ...
    def _add_bitmex_symbol(self, event):
        ...
        if symbol in self.bitmex_symbols:
            self._add_symbol(symbol, Exchange.bitmex)
            event.widget.delete(0, tk.END)

    def _add_symbol(self, symbol: str, exchange: Exchange):
        ...
        self.body_widgets["remove"][b_index] = tk.Button(self._table_frame, text="X", bg=BUTTON_DELETE_COLOR,
